...
const QuestionEdit = () => {
    ...
    fetch(`${BASE_URL}/admin/quiz/` + params.quizid, {
      ...
      if (data.status === 200) {
        data.json().then(result => {
          setQuizinfo(result)
          if (result.questions[questionid] !== undefined) {
            // questionDetail = result.questions;
            setquestionTitle(result.questions[questionid].question)
            setQuestionChoice(result.questions[questionid].answers)
            setQuestionTime(result.questions[questionid].time)
            setquestionanswer(result.questions[questionid].correctAnswer)
            setquestiontype(result.questions[questionid].Multi)
            setURL(result.questions[questionid].url)
            setPoints(result.questions[questionid].points)
            setquizPhoto(result.questions[questionid].thumbnail)
          }
        });
  ...
  function deleteQtion () {
    ...
    fetch(`${BASE_URL}/admin/quiz/${params.quizid}`, {
      ...
      if (data.status === 200) {
        data.json().then(result => {
          setErrorout(true);
          document.getElementById('error_out').innerText = 'Successful Delete!!!  '
  ...
  function save () {
    ...
    const newStruct = {
      ...
      thumbnail: quizinfo.questions[questionid].thumbnail,
      time: questionTime,
      url: url
    }
  ...
  return (
    <div>
      ...
      <div className = 'center_style'>
        ...
        <div className = 'card__header'>
          <div>
          <button className = 'button_style' onClick= {deleteQtion}>Delete This Question</button>
